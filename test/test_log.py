...
# 全局正则表达式：适配你的GeoServer日志格式（syslog封装+中文月份）
PATTERN_GEOSERVER_CORE = re.compile(
    r"""
    (?P<day>\d{2})\s+      # 改为 day
    (?P<mon_cn>\w+)\s+     # 中文月份
    (?P<time>\d{2}:\d{2}:\d{2})\s+  # 时间
    (?P<log_level>\w+)\s+  # 日志级别（INFO）
    \[(?P<module>[^\]]+)\]\s+-\s+  # 日志模块（geoserver.filters/geoserver.wfs）
    (?P<client_ip>\d+\.\d+\.\d+\.\d+)\s+  # 客户端IP
    "?(?P<http_method>\w+)\s+(?P<request_uri>[^"\s]+)"?\s+  # 请求方法+ URI
...
# 匹配WFS业务详情：Request: getFeature/typeName等（简化提取，适配日志格式）
...
class SecurityEngine:
    ...
    def analyze(log_entry: dict) -> dict:
        ...
        # 1. 检测 Java 注入特征 (针对 CVE-2024-36401)
...
class GeoServerLogParser:
    ...
    def parse_single_line(self, log_line: str, raw_log: Optional[str] = None) -> Optional[Dict]:
        ...
        # 基础结构化字典（轻量化安全审计字段）
        structured = {
            "timestamp": log_time,          # 标准化时间戳（智能体规则匹配核心）
            "client_ip": client_ip,         # 客户端IP（溯源/黑名单核心）
            "http_method": http_method,     # 请求方法
            "request_path": request_path,   # 请求路径（漏洞路径检测核心）
            "query_string": query_string,   # 查询参数（注入/漏洞检测核心）
            ...
            "response_time_ms": response_time_ms,  # 响应时间（DoS检测）
            "geo_service": "",              # GIS服务类型（WFS/WMS/STATIC）
            "geo_action": "",               # GIS操作（GetFeature/GetCapabilities）
            "geo_layer": "",                # GIS图层名（未授权访问检测）
            "geo_version": "",              # GIS版本（漏洞版本匹配）
            "raw_log": raw_log or log_line  # 原始日志（二次校验）
        }
...
def main():
    ...
    # 定义两个JSON输出路径：全部结果 + 仅威胁结果
    json_all_output_path = config.JSON_ALL_OUTPUT_PATH
    json_threat_output_path = config.JSON_THREAT_OUTPUT_PATH
    ...
    # 6. 将所有结果保存为JSON
    try:
        with open(json_all_output_path, "w", encoding="utf-8") as f:
            json.dump(results, f, ensure_ascii=False, indent=2)
    except Exception as e:
        pass
    
    # 7. 将仅威胁的结果保存为JSON（核心需求）
    try:
        ...
        with open(json_threat_output_path, "w", encoding="utf-8") as f:
    ...
    # 读取并返回威胁报告JSON内容
    try:
        with open(json_threat_output_path, "r", encoding="utf-8") as f:
            return f.read()
    except Exception as e:
        return "{}"
